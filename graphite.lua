...
_M.status = function()
	...
	if initialized == true then
		status['common_stat_fiber'] = common_stat_fiber
		status['stat_fiber'] = stat_fiber
...
local function wrap_func_with_stat_tail(stat_name, start_time, ...)
...
_M.wrap_func_with_stat = function(name, func)
	local stat_name = {
		avg = 'func_' .. name .. '_avg',
		min = 'func_' .. name .. '_min',
		max = 'func_' .. name .. '_max',
	...
	return function(...)
		return wrap_func_with_stat_tail(stat_name, fiber.time64(), func(...))
	end
end
